...
import { BookOpen, Bookmark, Clock, ArrowRight, Sparkles, ScrollText, Layers, User, ShieldCheck, Heart, BookCheck } from 'lucide-react';
    ...
    <div className="relative min-h-[92vh] flex flex-col items-center justify-center p-3 sm:p-6 md:p-8 bg-gradient-to-b from-[#EFEAE1] via-[#E8E2D5] to-[#DFD7C7]">
      ...
      <div className="relative w-full max-w-4xl flex flex-col lg:flex-row items-center gap-8 z-10">
        ...
        >
          ...
          <div className="pt-6 sm:pt-7 px-8 sm:px-12 pl-12 text-center relative z-20">
            ...
            <h1 className="font-display text-2xl sm:text-3xl md:text-4xl font-extrabold tracking-tight text-white uppercase mt-2 drop-shadow-[0_2px_10px_rgba(0,0,0,0.9)] leading-tight">
              The Danger of <br />
            ...
            <p className="font-serif italic text-xs sm:text-sm text-[#DFD7C8] font-medium mt-0.5 drop-shadow-sm max-w-xs mx-auto">
              Lessons from the Old and Young Prophet in Bethel
            </p>

            {/* Prominent Author Attribution on Hardcover */}
            <div className="mt-2 inline-flex items-center space-x-1.5 px-3 py-0.5 rounded-full bg-[#D4AF37]/20 border border-[#D4AF37]/50 text-[#F5E6C8] text-xs font-serif font-bold shadow-sm">
              <span>By {BOOK_METADATA.author}</span>
            </div>
          </div>

          {/* Centerpiece: Thematic Artwork with Gold Frame */}
          <div className="my-auto px-8 sm:px-12 pl-12 relative z-20">
            <div className="relative rounded-xl overflow-hidden shadow-2xl border-2 border-[#D4AF37]/70 group-hover:border-[#D4AF37] transition-all duration-300">
              <img 
                ...
                alt="Book cover illustration showing scriptures and path in Judean hills"
                className="w-full h-40 sm:h-48 object-cover object-center transform group-hover:scale-105 transition-transform duration-700"
          ...
          <div className="pb-5 sm:pb-6 px-8 sm:px-12 pl-12 text-center relative z-20">
        ...
        <div className="flex-1 w-full max-w-md lg:max-w-none flex flex-col justify-center space-y-4 bg-[#FAF8F5]/90 backdrop-blur-md p-6 sm:p-8 rounded-2xl border-2 border-[#5A5A40]/30 shadow-xl">
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              {onOpenAuthor && (
                <button
                  id="btn-cover-about-author"
                  onClick={onOpenAuthor}
                  className="inline-flex items-center space-x-1 text-xs font-mono font-black text-[#5A5A40] hover:text-black uppercase tracking-wider underline underline-offset-4 cursor-pointer"
                >
          ...
          {/* Author Card Preview with Emblem */}
          <div 
            onClick={onOpenAuthor}
            className="p-3.5 rounded-xl bg-[#EFE9DC] border border-[#D5CDC0] flex items-center justify-between cursor-pointer hover:bg-[#E8E0D0] transition-colors group shadow-2xs"
          >
            <div className="flex items-center space-x-3">
              <div className="w-11 h-11 rounded-xl bg-[#0B1120] border-2 border-[#D4AF37] flex items-center justify-center shrink-0 shadow-xs text-[#FACC15]">
                <BookCheck className="w-6 h-6" />
              </div>
              <div>
                <div className="text-xs font-black text-[#1A1A14] group-hover:text-[#5A5A40] transition-colors">
                  {BOOK_METADATA.author}
                </div>
                <div className="text-[11px] text-[#4A4A38] font-medium font-serif italic">
                  Also author of <em>“At the Throne of Grace”</em> & <em>Daily Grace</em>
                </div>
              </div>
            </div>
            <span className="text-xs font-bold text-[#5A5A40] uppercase tracking-wider group-hover:translate-x-1 transition-transform">
              View Bio →
            </span>
          </div>
          ...
          <div className="grid grid-cols-3 gap-2.5 py-0.5">
            <div className="p-2.5 rounded-xl bg-[#EFEAE0] border border-[#D5CEC0] text-center shadow-xs">
            ...
            <div className="p-2.5 rounded-xl bg-[#EFEAE0] border border-[#D5CEC0] text-center shadow-xs">
              ...
              <div className="text-xs font-black text-[#1A1A14]">41 Pages</div>
          ...
          <div className="space-y-2.5 pt-1">
            {savedPage > 1 && onResumeReading ? (
              <button
                ...
                className="w-full inline-flex items-center justify-center px-6 py-3.5 rounded-xl bg-[#202018] hover:bg-black text-white font-black text-base shadow-lg shadow-black/20 transition-all hover:scale-[1.02] active:scale-[0.98] cursor-pointer"
              >
            ...
            <button
              ...
              className={`w-full inline-flex items-center justify-center px-6 py-3.5 rounded-xl font-black text-base shadow-lg transition-all hover:scale-[1.02] active:scale-[0.98] cursor-pointer ${
                savedPage > 1
            ...
            <div className="grid grid-cols-2 gap-2">
              <button
                ...
                onClick={onOpenTOC}
                className="inline-flex items-center justify-center px-4 py-2.5 rounded-xl bg-[#EBE5D8] hover:bg-[#DFD8C8] text-[#1E1E14] border border-[#5A5A40]/40 font-bold text-xs uppercase tracking-wider transition-all cursor-pointer shadow-xs"
              >
                <Bookmark className="w-3.5 h-3.5 mr-1.5 text-[#5A5A40]" />
                <span>Contents (33 Ch)</span>
              </button>

              {onOpenAuthor && (
                <button
                  id="btn-author-legal-cover"
                  onClick={onOpenAuthor}
                  className="inline-flex items-center justify-center px-4 py-2.5 rounded-xl bg-[#EBE5D8] hover:bg-[#DFD8C8] text-[#1E1E14] border border-[#5A5A40]/40 font-bold text-xs uppercase tracking-wider transition-all cursor-pointer shadow-xs"
                >
                  <ShieldCheck className="w-3.5 h-3.5 mr-1.5 text-[#5A5A40]" />
